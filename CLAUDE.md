...
## デザイン（2026-09-19 全面刷新）

- 共通CSSは `site.css`（トップ・fc-package・fc-support）。ガイドは `guide/guide.css` が `site.css` を @import して記事用の部品を足す。**各ページに <style> を書かない**
- 文法：太いゴシック（Noto Sans JP 900）＋黄色の筆帯 `.kicker`＋マーカー `.marker`＋チケット/影付きカード＋ピンクのリボン。金の明朝は使わない（ロゴだけ Noto Serif）
- 色トークン：`--navy`（紺・基調）`--sun`（黄・強調とCTA）`--rose`（ピンク・ラベルと価格）`--cream`（薄いセクション背景）
- ヘッダー/フッターの markup は4ページで同一（ハンバーガー＋ `.nav-links`）。変えるときは全ページ同時に
- 写真：アリエス等、他社ブランドの写真は使わない。代表写真が届いたら `#rep` に入れる
